...
var dissectAutoHandleError = function(handleArgs, handleResults) {
  return dissect(handleArgs, function(callback, args, oldArgs) {
    var err = args[0];
    if (err) {
      callback(err);
    } else {
      handleResults(callback, slice.call(args, 1), oldArgs);
    }
  });
};
...
var pipe = function() {
  return arguments.length ? reduce.call(arguments, pipe0) : echo;
};
